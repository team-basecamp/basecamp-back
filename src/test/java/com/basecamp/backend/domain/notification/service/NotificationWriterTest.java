 ...
 * 알림 저장(멱등) 단위 테스트(#92).
 *
 * <p>동시 삽입 경합 시의 유니크 제약 위반은 DB 가 강제하므로 여기서 재현하지 않는다. 이 테스트는
 * 순차 중복을 걸러내는 존재 검사와, 저장되는 내용이 알림 종류에서 올바르게 파생되는지를 확인한다.</p>
 */
...
class NotificationWriterTest {
	...
	void saveIfAbsent_없으면_저장한다() {
		// given
		given(notificationRepository.existsByUserIdAndTypeAndTargetId(
				USER_ID, NotificationType.RESERVATION_CONFIRMED, TARGET_ID)).willReturn(false);
		given(notificationRepository.save(any(Notification.class))).willAnswer(inv -> inv.getArgument(0));

		// when
		Optional<Notification> result = notificationWriter.saveIfAbsent(
	...
	void saveIfAbsent_이미있으면_저장하지않는다() {
		...
		given(notificationRepository.existsByUserIdAndTypeAndTargetId(
				USER_ID, NotificationType.RESERVATION_D1, TARGET_ID)).willReturn(true);
	...
	void saveIfAbsent_target이null이면_검사를건너뛴다() {
		...
		// when
		Optional<Notification> result = notificationWriter.saveIfAbsent(
				USER_ID, NotificationType.CAMP_OWNER_APPROVED, null, null);

		// then
		assertThat(result).isPresent();
		verify(notificationRepository, never())
				.existsByUserIdAndTypeAndTargetId(any(), any(), any());
