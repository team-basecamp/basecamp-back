 ...
 *
 * <p>SSE push 로직은 {@code NotificationServiceTest} 가 다루고, 여기서는 조회/카운트/읽음 API 의
 * 요청·응답 조립과 예외 매핑만 확인한다. 보안 필터는 이 계층의 관심사가 아니므로 {@code addFilters = false} 로
 * 끄고, {@code @AuthenticationPrincipal} 이 읽을 인증만 직접 세팅한다({@code AuthControllerTest} 와 같은 방식).</p>
 */
...
class NotificationControllerTest {
	...
	private NotificationResponse sampleResponse() {
		return new NotificationResponse(
				NOTIFICATION_ID, NotificationType.RESERVATION_CONFIRMED,
				"'해운대 오토캠핑장' 예약이 확정되었습니다.",
				NotificationTargetType.RESERVATION, 42L, false,
