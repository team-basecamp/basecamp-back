 ...
 *
 * <p>{@code ROLE_ADMIN} 인가는 {@code SecurityConfig} 의 관심사이므로 {@code addFilters = false} 로 끄고,
 * 요청/응답 조립·검증과 예외 매핑만 확인한다({@code AdminUserControllerTest} 와 같은 방식).</p>
 */
...
class AdminPostControllerTest {
	...
	private ReportedPostResponse sampleReport() {
		return new ReportedPostResponse(
				10L, POST_ID, "예약 양도합니다", "ACTIVE", "RESERVATION_TRANSFER",
				"INAPPROPRIATE", "부적절한 사진", "PENDING", 7L, "신고자",
	...
	void findReports_200과_신고목록을반환한다() throws Exception {
		...
		// when & then
		mockMvc.perform(get("/api/v1/admin/posts/reports"))
	...
	void findReports_status쿼리파라미터를_서비스로전달한다() throws Exception {
		...
		// when & then
		mockMvc.perform(get("/api/v1/admin/posts/reports").param("status", "ACCEPTED"))
				.andExpect(status().isOk());
	...
	void blindPost_204와_사유를_서비스로전달한다() throws Exception {
		// given & when & then
		mockMvc.perform(post("/api/v1/admin/posts/{postId}/blind", POST_ID)
	...
	void blindPost_없는게시글_404() throws Exception {
		...
		willThrow(new BusinessException(ErrorCode.POST_NOT_FOUND))
				.given(adminPostService).blindPost(POST_ID, "사유");
