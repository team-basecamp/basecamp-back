...
class AdminUserControllerTest {
	...
	void blacklistUser_204와_사유를_서비스로전달한다() throws Exception {
		// given & when & then
		mockMvc.perform(post("/api/v1/admin/users/{userId}/blacklist", USER_ID)
	...
	void blacklistUser_이미제재된회원_409() throws Exception {
		...
		willThrow(new BusinessException(ErrorCode.USER_ALREADY_BLACKLISTED))
				.given(adminUserService).blacklistUser(USER_ID, "사유");
	...
	void findBlacklistedUsers_200과_제재회원목록을반환한다() throws Exception {
		// given
		BlacklistedUserResponse response = new BlacklistedUserResponse(
				USER_ID, "user@example.com", "camper", "KAKAO", "어뷰징", LocalDateTime.of(2026, 7, 9, 9, 0));
		...
		mockMvc.perform(get("/api/v1/admin/users/blacklist"))
				...
				.andExpect(jsonPath("$.content[0].blacklistReason").value("어뷰징"))
