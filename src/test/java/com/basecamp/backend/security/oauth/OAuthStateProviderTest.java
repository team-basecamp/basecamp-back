 ...
 * 만료 검증을 위해 고정 {@link Clock} 을 주입한다.
 */
class OAuthStateProviderTest {
	...
	private OAuthStateProvider providerAt(String secret, Instant instant) {
		return new OAuthStateProvider(new JwtProperties(secret, 1800000L, 1209600000L), Clock.fixed(instant, ZONE));
	}
	...
	void verify_다른secret_false() {
		...
		String foreignState = providerAt("another-secret-key-completely-different-0987654321", NOW).issue();
