...
class JwtTokenProviderTest {
	...
	void createToken_같은사용자로_두번발급시_jti가_서로다르다() {
		// given & when
		String first = provider.getJti(provider.parseClaims(provider.createRefreshToken(USER_ID, ROLE)));
		String second = provider.getJti(provider.parseClaims(provider.createRefreshToken(USER_ID, ROLE)));
