...
class JwtAuthenticationFilterTest {
	...
	private void withBearer(String token) {
		request.addHeader(JwtAuthenticationFilter.AUTHORIZATION_HEADER, JwtAuthenticationFilter.BEARER_PREFIX + token);
	}
	...
	void doFilter_폐기된access토큰_인증하지않고_INVALID_TOKEN을남긴다() throws Exception {
		...
		withBearer(jwtTokenProvider.createAccessToken(USER_ID, ROLE));
		given(tokenBlacklistCache.isBlacklisted(org.mockito.ArgumentMatchers.anyString())).willReturn(true);
