 ...
 *
 * <p>봉투 계약({@code success}/{@code message}/{@code data}) 자체는 개별 컨트롤러가 아니라
 * 이 advice 의 책임이므로, 여기서 한 번에 못박는다. 각 컨트롤러 슬라이스 테스트는
 * {@code $.success} 만 확인해 "성공 봉투로 감쌌다"는 경계 사실만 검증한다.</p>
 */
class ApiResponseWrapAdviceTest {
	...
	void beforeBodyWrite_일반DTO_success와OK메시지봉투로감싼다() {
		...
		Object result = advice.beforeBodyWrite(
				body, null, MediaType.APPLICATION_JSON, null, null, newResponse());
	...
	void beforeBodyWrite_null본문_감싸지않는다() {
		// given & when
		Object result = advice.beforeBodyWrite(
				null, null, MediaType.APPLICATION_JSON, null, null, newResponse());
	...
	void beforeBodyWrite_String본문_JSON봉투로직렬화() throws Exception {
		...
		Object result = advice.beforeBodyWrite(
				"동기화 완료", null, MediaType.TEXT_PLAIN, null, null, response);
