 ...
 * {@link ApiResponse} 봉투로 자동으로 감싼다.
 *
 * <p>덕분에 각 컨트롤러는 순수 DTO 만 반환하고, 오류 응답({@code GlobalExceptionHandler})과
 * 동일한 {@code { success, message, data }} 구조가 전 엔드포인트에 보장된다.</p>
 *
 * <ul>
 *   <li>springdoc(Swagger) 컨트롤러는 basePackages 밖이라 감싸지 않는다 → API 문서가 깨지지 않는다.</li>
 *   <li>이미 {@link ApiResponse} 인 본문(예외 핸들러 출력·수동 래핑)은 그대로 통과시킨다.</li>
