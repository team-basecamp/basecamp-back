 ...
 *
 * <p>서버가 {@code nonce.expiry.HMAC(nonce.expiry)} 형태의 서명된 문자열을 발급하고, 콜백에서 서명·만료만
 * 검증한다. 별도 저장소 없이 위조를 막을 수 있어 무상태(JWT) 설계 및 서버 재시작/스케일아웃에 부합한다.
 * 서명 키는 {@code jwt.secret}(HMAC-SHA256)을 재사용한다.</p>
 */
@Component
