...
/**
 * 시큐리티 필터 단계(EntryPoint / AccessDeniedHandler)에서 발생한 예외를
 * {@link ApiResponse} JSON 형식으로 직렬화해 응답한다.
 *
 * <p>필터 단계 예외는 {@code @RestControllerAdvice}가 처리하지 못하므로 별도 직렬화가 필요하다.
 * 컨트롤러/전역 예외 핸들러와 같은 {@code { success, code, message }} 봉투를 써서 응답 구조를 통일한다.</p>
 */
