 ...
 * jwt.* 설정값 바인딩.
 * <ul>
 *     <li>{@code jwt.secret} : HMAC 서명 키 (HMAC-SHA 최소 키 길이 충족을 위해 32자 이상)</li>
 *     <li>{@code jwt.access-token-expiration} : Access Token 만료시간(ms, 양수)</li>
 *     <li>{@code jwt.refresh-token-expiration} : Refresh Token 만료시간(ms, 양수)</li>
 * </ul>
 *
 * <p>잘못된 값(빈/짧은 secret, 0 이하 TTL)이 {@link JwtTokenProvider}로 전달되기 전에
 * 바인딩 시점에서 fail-fast 로 차단하기 위해 Bean Validation 을 적용한다.</p>
 */
@Getter
@Validated
