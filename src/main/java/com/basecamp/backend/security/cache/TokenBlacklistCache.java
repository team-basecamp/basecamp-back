 ...
 * access 토큰 검증은 매 API 요청마다 일어나므로 DB 를 직접 때리면 무상태 JWT 의 이점이 사라진다.</p>
 *
 * <p>키는 토큰의 남은 수명만큼만 살아 있다가 TTL 로 자동 삭제된다. 만료된 토큰은 서명 검증 단계에서 이미 걸러지므로
 * 캐시에 남겨둘 이유가 없다.</p>
 *
 * <p><b>장애 정책:</b> 조회 실패는 <i>fail-open</i>(경고 로그 후 통과)이다. Redis 가 죽었다고 전체 인증을 막으면
 * 단일 장애점이 된다. 이때 동작은 Step 8 이전, 즉 "access 토큰은 만료 전까지 유효"로 퇴화할 뿐이다.
 * 반면 <b>쓰기 실패는 삼키지 않는다</b>. 조용히 넘기면 로그아웃했는데 토큰이 살아 있는 상태가 되기 때문이다.</p>
 */
@Slf4j
@Component
...
public class TokenBlacklistCache {
	 ...
	 *
	 * <p>실패 시 예외가 그대로 전파된다. 호출자(로그아웃/탈퇴/회전)는 폐기가 실패했음을 알아야 한다.</p>
	 */
