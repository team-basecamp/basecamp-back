 ...
 *
 * <p>제재의 본질은 "이 토큰을 죽여라"가 아니라 "이 사람의 모든 토큰을 죽여라"다. 무상태 JWT 라 서버가 발급된 토큰을 보관하지 않으므로 대상 회원의 {@code
 * jti} 를 알 수 없고, 따라서 토큰 단위 denylist({@link TokenBlacklistCache}) 로는 제재를 표현할 수 없다. 대신 회원 식별자로 표시해 두고
 * 인증 필터가 매 요청 확인한다(#18).
 *
 * <p><b>TTL 은 access 토큰 수명이면 충분하다.</b> 그 시간이 지나면 살아 있는 access 토큰은 전부 제재 이후 발급분인데, 제재 중에는 소셜 로그인과 토큰
 * 재발급이 모두 {@code users.status = BLACKLISTED} 로 막혀 있어 애초에 발급되지 않는다.
 *
