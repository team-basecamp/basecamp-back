 ...
 *
 * <p>제재(#18)·업체 승격(#53)의 본질은 "이 토큰을 죽여라"가 아니라 "이 사람의 (지금까지 발급된) 토큰을 죽여라"다.
 * 무상태 JWT 라 서버가 발급된 토큰을 보관하지 않으므로 대상 회원의 {@code jti} 를 알 수 없고, 따라서 토큰 단위
 * denylist({@link TokenBlacklistCache}) 로는 표현할 수 없다. 대신 회원 식별자로 표시해 두고 인증 필터가 매 요청 확인한다.</p>
 *
 * <p><b>값은 "무효화 시각(revokedAt, epoch 초)"이다.</b> 존재 여부만 보던 이전 방식은 무효화된 회원의 <i>모든</i>
 * 토큰을 막아, 무효화 직후 재로그인해 받은 <b>새 토큰까지</b> 거부했다(#119). 이제 {@code isRevoked} 는 토큰의 발급
 * 시각(iat)과 revokedAt 을 비교해 <b>revokedAt 이전에 발급된 토큰만</b> 거부한다.</p>
 * <ul>
 *   <li>제재: 재로그인이 {@code users.status = BLACKLISTED} 로 막혀 새 토큰이 안 나오므로 옛 토큰만 죽는다(종전과 동일).</li>
 *   <li>승격: DB status 는 정상이라 재로그인이 되고, 그때 받은 새 토큰(iat ≥ revokedAt)은 통과한다.</li>
 * </ul>
 *
 * <p><b>TTL 은 access 토큰 수명이면 충분하다.</b> 그 시간이 지나면 revokedAt 이전 발급 토큰은 전부 만료된다.</p>
 *
...
public class UserRevocationCache {
	...
	public UserRevocationCache(StringRedisTemplate redisTemplate, JwtProperties jwtProperties, Clock clock) {
	...
	/**
	 * 지금 이 시각 이전에 발급된 회원의 access 토큰을 무효화한다. 이후 재로그인해 받은 토큰은 영향받지 않는다.
	 * 실패 시 예외가 전파되어 제재/승격 트랜잭션이 롤백된다.
	 */
	 ...
	 *
	 * <p>iat 와 revokedAt 은 모두 초 단위(JWT 표준)이고 비교는 {@code <} 다. 같은 초에 발급된 토큰은 통과시켜,
	 * 무효화 직후 재로그인으로 발급된 토큰이 초 절삭 때문에 막히는 것을 피한다(옛 토큰이 최대 1초 더 사는 것은 무해).</p>
	 *
