 ...
 *
 * <p><b>왜 필요한가:</b> 웹훅 엔드포인트는 포트원 서버가 호출해야 하므로 우리 JWT 인증을 걸 수 없어 공개돼 있다.
 * 서명을 검증하지 않으면 누구나 "결제 완료" 웹훅을 흉내내 예약을 공짜로 확정시킬 수 있다.
 * 서명 검증이 이 엔드포인트의 유일한 인증 수단이다.</p>
 *
 * <p>검증 절차 (Standard Webhooks):</p>
 * <ol>
 *   <li>서명 대상 문자열을 {@code {webhook-id}.{webhook-timestamp}.{본문}} 으로 조립한다.</li>
 *   <li>시크릿({@code whsec_} 접두어를 뗀 base64)을 키로 HMAC-SHA256 을 계산해 base64 로 만든다.</li>
 *   <li>{@code webhook-signature} 헤더의 {@code v1,<서명>} 목록 중 하나와 일치하면 통과한다.</li>
 *   <li>타임스탬프가 현재 시각과 5분 이상 벌어지면 거부한다(재전송 공격 차단).</li>
 * </ol>
 */
@Slf4j
@Component
