 ...
 *
 * <p>포트원 응답에는 여기 선언하지 않은 필드가 많고 버전이 올라가며 더 늘어난다.
 * {@code @JsonIgnoreProperties(ignoreUnknown = true)} 로 무시해야 필드 추가가 우리 서버를 깨뜨리지 않는다
 * (포트원 문서도 "모르는 필드는 무시하라"고 안내한다).</p>
 */
@JsonIgnoreProperties(ignoreUnknown = true)
public record PortOnePayment(
        String id,                 // 우리가 넘긴 결제 건 ID (= payments.pg_payment_id)
        String status,             // READY / PENDING / VIRTUAL_ACCOUNT_ISSUED / PAID / FAILED / CANCELLED / PARTIAL_CANCELLED
    ...
    /**
     * 결제 수단. {@code type} 이 판별자다: {@code PaymentMethodCard}, {@code PaymentMethodEasyPay},
     * {@code PaymentMethodTransfer}, {@code PaymentMethodVirtualAccount}, {@code PaymentMethodMobile} 등.
