 ...
 *
 * <p>포트원(PortOne) V2 연동 기준 상태 흐름:</p>
 * <pre>
 *   READY ──(포트원 조회 결과 PAID)──▶ PAID ──(취소 API 성공)──▶ REFUNDED
 *     └────(포트원 조회 결과 FAILED)──▶ FAILED ──(재시도)──▶ READY
 * </pre>
 *
 * <p>READY 행은 결제창을 띄우기 전에 미리 잡아두는 자리다. 돈이 오간 기록이 아니므로
 * 매출 통계는 반드시 {@code status = 'PAID'} 로 걸러서 집계해야 한다.</p>
 */
@Entity
@Table(name = "payments")
@Getter
...
public class Payment {
    ...
    @Builder
    private Payment(Reservation reservation, Long amount, String pgPaymentId, String pgTxId, String pgProvider,
                    PaymentMethod paymentMethod, PaymentStatus status, LocalDateTime paidAt,
                    LocalDateTime refundedAt, String failureReason, LocalDateTime createdAt) {
     ...
     *
     * @param requestedMethod 사용자가 고른 결제 수단. 결제 완료 시 포트원이 알려주는 실제 수단으로 덮인다.
     *                        미리 담아두는 이유는 결제가 실패했을 때 "무엇으로 시도했는지"가 남아야 하기 때문이다.
     */
    public static Payment ready(Reservation reservation, String pgPaymentId, PaymentMethod requestedMethod) {
     ...
     *
     * <p>예약당 결제 행은 1개(유니크 키)라 재시도할 때 행을 새로 만들 수 없다. 결제 건 ID를
     * 새로 발급해 갈아끼우는 이유는, 포트원에서 같은 ID로 두 번 결제를 시도할 수 없기 때문이다.</p>
     */
     ...
     *
     * <p>이미 PAID 면 아무 것도 하지 않는다. 완료 확인 API와 웹훅이 같은 결제 건에 대해
     * 순서 없이 도착하기 때문에, 두 경로 모두 이 메서드를 거쳐 멱등하게 처리한다.</p>
     *
     * @return 이번 호출로 READY → PAID 전이가 실제로 일어났으면 true
     */
    public boolean markPaid(PaymentMethod method, String pgTxId, String pgProvider, LocalDateTime paidAt) {
        if (this.status == PaymentStatus.PAID) {
            return false;
        }
