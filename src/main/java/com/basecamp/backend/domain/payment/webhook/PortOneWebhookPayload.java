 ...
 *
 * <p>웹훅은 "이 결제 건에 변화가 있었다"는 알림일 뿐, 결제 결과 자체가 아니다.
 * 그래서 여기 담긴 값으로 결제를 확정하지 않고, {@code paymentId} 로 포트원에 다시 조회해
 * 실제 상태와 금액을 확인한다({@code PaymentService.syncFromPortOne}).</p>
 *
 * @param type 이벤트 종류. {@code Transaction.Paid}, {@code Transaction.Failed},
 *             {@code Transaction.Cancelled}, {@code Transaction.VirtualAccountIssued} 등.
 */
