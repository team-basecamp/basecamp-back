...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
     ...
     *
     * <p>결제 완료는 두 경로로 들어온다: 프론트의 완료 확인 요청과 포트원 웹훅. 둘은 순서 없이,
     * 때로는 거의 동시에 도착한다. 잠그지 않으면 양쪽이 나란히 READY 를 읽고 각자 PAID 로 만들면서
     * 예약 확정 로직이 두 번 돈다. 행 잠금으로 한 쪽이 끝난 뒤에 다른 쪽이 읽게 만들면,
     * 뒤에 온 쪽은 이미 PAID 인 것을 보고 조용히 넘어간다.</p>
     */
