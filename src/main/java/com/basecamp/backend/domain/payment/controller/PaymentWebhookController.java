 ...
 * 포트원 웹훅 수신 엔드포인트.
 *
 * <p><b>인증이 없는 대신 서명으로 지킨다.</b> 포트원 서버가 호출하므로 우리 JWT를 요구할 수 없어
 * {@code SecurityConfig} 에서 공개(permitAll)로 열려 있다. 대신 모든 요청은
 * {@link PortOneWebhookVerifier} 의 HMAC 서명 검증을 통과해야 하며, 실패하면 401 로 끊는다.</p>
 *
 * <p><b>본문을 문자열로 받는 이유:</b> 서명은 전송된 <b>바이트 그대로</b>에 대해 계산된다.
 * DTO로 역직렬화한 뒤 다시 직렬화하면 공백·필드 순서가 달라져 서명이 반드시 깨진다.
 * 그래서 원문을 받아 검증부터 하고, 그 다음에 파싱한다.</p>
 */
@Slf4j
@RestController
