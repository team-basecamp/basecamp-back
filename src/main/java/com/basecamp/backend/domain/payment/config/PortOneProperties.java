 ...
 *
 * <p>{@code storeId} / 채널 키는 프론트 결제창 호출에 그대로 노출되는 공개 값이지만,
 * {@code apiSecret} 과 {@code webhookSecret} 은 절대 응답에 실어 보내면 안 된다.
 * 결제 준비 응답({@code PaymentPrepareResponse})이 앞의 것만 담는 이유다.</p>
 *
...
public record PortOneProperties(
    ...
    /**
     * 결제 수단별 채널 키. 포트원은 PG사·간편결제사마다 채널을 따로 만들게 되어 있어,
     * 어떤 수단으로 결제하느냐에 따라 결제창에 넘길 채널 키가 달라진다.
     *
     ...
     *
     * <p>설정되지 않은 수단으로 결제를 시도하면 {@code PG_NOT_CONFIGURED} 로 끊는다. 빈 채널 키를 그대로
     * 프론트에 내려보내면 포트원 결제창이 알아보기 어려운 오류로 죽어 원인 파악이 오래 걸린다.</p>
     */
    public String resolveChannelKey(PaymentMethod method) {
        ...
        if (key == null || key.isBlank()) {
            throw new BusinessException(ErrorCode.PG_NOT_CONFIGURED,
                    "%s 결제 채널이 설정되지 않았습니다.".formatted(method));
        }
        return key;
    }
