 ...
 *
 * <p>테스트 모드라도 호출 경로·인증·응답 구조는 실거래와 동일하다. 콘솔에서 테스트 채널을 쓰면
 * 실제 승인만 일어나지 않을 뿐, 우리 서버 코드는 그대로 운영에 넘어간다.</p>
 *
 * <p><b>클라이언트가 예외를 던지는 이유:</b> 지오코딩처럼 실패해도 넘어갈 수 있는 부가 기능과 달리,
 * 결제 검증은 실패하면 반드시 멈춰야 한다. 포트원 응답을 못 받았는데 "결제됐다"고 처리하면
 * 돈을 안 받고 예약을 확정하게 된다. 그래서 모든 실패는 예외로 올린다.</p>
 */
@Slf4j
@Component
public class PortOneClient {
    ...
    public PortOneClient(RestClient restClient, PortOneProperties properties, ObjectMapper objectMapper) {
     ...
     *
     * <p>프론트 응답은 사용자가 위조할 수 있으므로, 결제 완료 여부와 금액은 반드시 이 조회 결과로 판단한다.</p>
     */
     ...
     *
     * <p>이미 취소된 건에 다시 취소를 걸면 포트원이 {@code PaymentAlreadyCancelledError} 를 준다.
     * 이는 우리가 원하는 최종 상태와 같으므로 성공으로 간주한다 — 스케줄러 재실행이나
     * 웹훅 중복 수신으로 취소가 두 번 호출돼도 환불 처리가 막히지 않아야 한다.</p>
     */
    public void cancelPayment(String pgPaymentId, String reason) {
        String url = properties.apiBaseUrl() + "/payments/" + encodePathSegment(pgPaymentId) + "/cancel";
