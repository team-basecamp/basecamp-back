...
public record MyPostCursorResponse(
        @Schema(description = "이번 페이지의 내 게시글 목록")
        List<MyPostResponse> content,

        @Schema(description = "다음 페이지 존재 여부", example = "true")
        boolean hasNext,

        @Schema(description = "다음 요청에 cursor로 그대로 실어 보낼 값. hasNext가 false면 null.")
        String nextCursor
) {
    ...
    public static MyPostCursorResponse of(List<Post> lookahead, int limit, Map<Long, Integer> commentCounts) {
        ...
        List<MyPostResponse> content = posts.stream()
