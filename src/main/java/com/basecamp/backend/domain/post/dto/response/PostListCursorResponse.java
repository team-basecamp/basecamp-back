...
public record PostListCursorResponse(
    @Schema(description = "이번 페이지의 게시글 목록") List<PostListResponse> content,
  ...
  public static PostListCursorResponse of(
    ...
    List<PostListResponse> content =
        posts.stream()
            .map(
                post ->
                    PostListResponse.from(post, commentCounts.getOrDefault(post.getPostId(), 0)))
            .toList();
    ...
    return new PostListCursorResponse(content, hasNext, nextCursor);
  }
