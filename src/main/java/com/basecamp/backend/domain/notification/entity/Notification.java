 ...
 *
 * <p>{@code user_id} 는 감사 목적의 참조라 연관 매핑 대신 식별자만 들고 있다({@code CampOwnerApplication} 과 같은 방식).
 * 알림을 읽을 때 {@code User} 를 함께 조회할 이유가 없다. {@code target_type} + {@code target_id} 는 알림이 가리키는
 * 대상(예약/신청 등)으로, 프론트가 상세 화면으로 이동할 때 쓴다(V2 의 B안, FK 없음 — 어떤 도메인이든 가리킬 수 있게).</p>
 */
@Getter
@Entity
...
public class Notification {
	...
	public static Notification create(Long userId, NotificationType type, String message, Long targetId) {
