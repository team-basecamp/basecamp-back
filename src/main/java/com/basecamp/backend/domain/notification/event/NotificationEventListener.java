 ...
 *
 * <p>{@code AFTER_COMMIT} 이라 트리거(예약 확정/거절, 업체 승인/반려)가 <b>확정된 뒤에만</b> 알림이 나간다.
 * 알림 발생 중 오류가 나도 이미 커밋된 트리거에는 영향이 없어야 하므로 예외를 삼키고 로그만 남긴다 —
 * 알림은 best-effort 다.</p>
 */
@Slf4j
@Component
...
public class NotificationEventListener {
	...
	public void handle(NotificationEvent event) {
		try {
			...
			log.warn("알림 발생 실패: userId={}, type={}, targetId={}",
					event.userId(), event.type(), event.targetId(), e);
		}
	}
}
