 ...
 * 인증 필터가 매 요청 조회하는 표시는 {@link UserRevocationCache}(Redis)가 담당한다.</p>
 *
 * <p>제재 중에는 세 경로가 모두 막힌다: access 토큰(필터), 토큰 재발급, 소셜 재로그인.</p>
 */
@Service
@Transactional
...
public class AdminUserService {
	 ...
	 *
	 * <p><b>Redis 를 먼저 쓴다.</b> 뒤이어 DB 가 롤백되면 캐시에만 표시가 남지만, 그건 "죽지 않아야 할 토큰이 죽는"
	 * 방향이라 안전하고 TTL 로 자동 정리된다. 반대 순서였다면 커밋과 캐시 반영 사이에 제재된 회원의 요청이 통과한다.</p>
	 */
	 ...
	 *
	 * <p><b>DB 를 먼저 바꾼다.</b> Redis 삭제가 실패하면 예외가 전파되어 트랜잭션이 롤백되고 제재 상태가 유지된다.
	 * 반대 순서였다면 캐시만 지워져 제재가 사실상 풀린 채 DB 는 BLACKLISTED 로 남는다.</p>
	 */
	...
	public Page<BlacklistedUserResponse> findBlacklistedUsers(Pageable pageable) {
		return userRepository.findByStatus(UserStatus.BLACKLISTED, pageable)
