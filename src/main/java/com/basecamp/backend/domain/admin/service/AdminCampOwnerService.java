 ...
 *
 * <p>승인은 <b>신청 상태 변경 + 회원 권한 승격 + 구 access 토큰 무효화</b>가 한 트랜잭션으로 묶인다.
 * 셋 중 하나라도 실패하면 전부 되돌아가야 한다 — "승격됐는데 구 토큰이 살아 있는" 어중간한 상태를 만들지 않는다.</p>
 */
@Service
@Transactional
...
public class AdminCampOwnerService {
	...
	public Page<CampOwnerApplicationResponse> findApplications(ApplicationStatus status, Pageable pageable) {
	...
	/**
	 * 탈퇴 회원은 승격 대상이 아니다. 제재 회원도 마찬가지다 — 제재를 풀지 않은 채 권한만 올리면
	 * 해제되는 순간 업체 권한을 그대로 갖게 된다.
	 */
