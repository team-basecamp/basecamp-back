...
public class AdminCampOwnerService {
   ...
   *
   * <p><b>DB 를 먼저 바꾸고 Redis 를 나중에 쓴다.</b> {@code role} 은 access 토큰 클레임에 들어 있어 승격 직후에도 구 토큰은 {@code
   * CUSTOMER} 다. 무효화해야 사용자가 재로그인해 새 권한을 받는다. {@code revoke()} 가 실패하면 예외가 전파되어 승격도 함께 롤백된다.
   *
   ...
   * docs/camp-owner-promotion.md} §5).
   */
