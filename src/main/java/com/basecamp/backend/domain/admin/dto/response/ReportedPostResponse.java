 ...
 *
 * <p>{@code post}/{@code reporter} LAZY 연관에 접근하므로 {@link #from} 은 두 연관이 함께 로딩된 상태에서
 * 호출돼야 한다({@code PostReportRepository.findByStatus} 의 {@code @EntityGraph} 참고).</p>
 */
...
public record ReportedPostResponse(
		...
		@Schema(description = "신고 대상 게시글 ID", example = "42")
		Long postId,
		...
		@Schema(description = "신고 사유 코드", example = "INAPPROPRIATE")
		String reason,

		@Schema(description = "신고 상세 내용", example = "부적절한 사진이 포함되어 있습니다.")
		String description,

		@Schema(description = "신고 처리 상태 (PENDING / ACCEPTED / REJECTED)", example = "PENDING")
		String reportStatus,

		@Schema(description = "신고자 회원 ID", example = "7")
		Long reporterId,

		@Schema(description = "신고자 닉네임", example = "캠퍼")
		String reporterNickname,

		@Schema(description = "신고 접수 일시")
