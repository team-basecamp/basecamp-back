 ...
 * {@code @EntityGraph} 로 함께 로딩한 뒤에 매핑해야 한다. 다른 경로에서 이 DTO 를 만들면 N+1 이 된다.</p>
 *
 * <p>제재 정보({@code blacklistReason}, {@code blacklistedAt})와 탈퇴 시각({@code deletedAt})은 해당 상태일 때만
 * 값이 있다. 탈퇴 사유는 회원이 직접 쓴 텍스트라 목록에 싣지 않는다.</p>
 */
...
public record AdminUserResponse(
		...
		@Schema(description = "이메일", example = "camper@example.com")
		String email,

		@Schema(description = "닉네임", example = "캠퍼")
		String nickname,

		@Schema(description = "프로필 이미지 URL. 없으면 null", example = "https://cdn.example.com/profile/1.png")
		...
		@Schema(description = "제재 일시. 제재 중일 때만 값이 있다")
		LocalDateTime blacklistedAt,

		@Schema(description = "탈퇴 일시. 탈퇴 회원만 값이 있다")
