 ...
 *
 * <p>경로가 {@code /api/v1/admin} 아래가 아니므로 {@code SecurityConfig} 의 URL 규칙으로는 권한이 좁혀지지 않는다.
 * 신청은 일반 회원만 하는 행위라 메서드에 {@code @PreAuthorize} 를 건다.</p>
 */
...
public class CampOwnerApplicationController {
	...
	@Operation(summary = "내 신청 상태 조회",
			description = "본인의 가장 최근 신청 1건을 조회한다. 신청 이력이 없으면 404(CO001). "
					+ "승격된 뒤에도 승인 이력을 볼 수 있어야 하므로 권한을 좁히지 않는다.")
	@GetMapping("/me")
	public ResponseEntity<CampOwnerApplicationResponse> findMyApplication(@AuthenticationPrincipal AuthUser user) {
