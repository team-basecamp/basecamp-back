...
public class CampOwnerApplicationService {
	 ...
	 * 업체 전환을 신청한다.
	 *
	 * <p><b>동시 신청 방어:</b> {@code existsByUserIdAndStatus} 조회와 INSERT 사이에 다른 요청이 끼어들 수 있다.
	 * 최종 방어선은 DB 의 {@code uq_coa_user_pending} 유니크 제약이고, 위반은 "이미 심사 중"과 같은 상황이므로
	 * {@code CO002} 로 변환한다. 제약 위반을 즉시 보려면 {@code saveAndFlush} 여야 한다
	 * (지연 flush 면 트랜잭션 커밋 시점에 터져 여기서 잡지 못한다).</p>
	 */
	public CampOwnerApplicationResponse apply(Long userId, CampOwnerApplicationRequest request) {
		...
		CampOwnerApplication application = CampOwnerApplication.submit(
