 ...
 *
 * <p>코드 릴레이 방식: 인가 코드를 받아 {@code code → 토큰 교환(client_secret 포함) → user-info 조회} 를 수행하고,
 * provider별 응답을 {@link OAuthUserInfo} 로 정규화해 반환한다.</p>
 * <p>카카오/구글/네이버 구현체가 각각 이 인터페이스를 구현하며, {@link SocialClientResolver} 가 {@link #provider()} 로 선택한다.</p>
 */
