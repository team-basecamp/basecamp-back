 ...
 *
 * <p>조건이 비어 있으면 {@code null} 을 돌려준다. {@code Specification.and(null)} 은 해당 조건을 무시하므로,
 * 호출부는 파라미터 유무를 분기하지 않고 그대로 이어붙일 수 있다({@code CampSpecs} 와 같은 방식).</p>
 */
public class UserSpecs {
	 ...
	 * 닉네임 또는 이메일 부분 일치.
	 *
	 * <p>앞 와일드카드(`%keyword%`) 때문에 인덱스를 타지 못하고 풀 스캔이 된다. 관리자 전용 화면이고 회원 규모가
	 * 크지 않아 감수한다. 회원 수가 늘어 문제가 되면 전문 검색 인덱스로 옮긴다.</p>
	 */
