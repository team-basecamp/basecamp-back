...
public interface UserRepository extends JpaRepository<User, Long>, JpaSpecificationExecutor<User> {
	 ...
	 *
	 * <p>목록이 프로필 이미지를 노출하므로 {@code profileImage} 를 함께 가져온다. 오버라이드 없이 LAZY 연관에
	 * 접근하면 페이지 크기만큼 SELECT 가 추가로 나간다(N+1). {@code image_id} FK 를 users 가 들고 있는
	 * to-one 연관이라 LEFT JOIN 한 방으로 끝나고, 컬렉션 페치가 아니므로 페이징도 DB 에서 처리된다.</p>
	 */
	@Override
