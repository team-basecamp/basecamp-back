 ...
 *
 * <p><b>일괄 UPDATE 대신 건별로 변경:</b> 환불이 포트원 취소 API 호출을 동반하도록 변경됨.
 * 포트원 API를 추가함으로써 환불시스템 로직에 추가됨으로써 이에 맞게 스케줄러 변경 필요
 * 물리적인 금액과 DB의 금액이 맞지 않아 수정</p>
 *
 * <p>스케줄러 메서드 자체에는 트랜잭션을 걸지 않는다. 건별 트랜잭션은
 * {@link ReservationService#autoRejectExpired} 가 각자 연다 — 한 건의 실패가 나머지를 되돌리지 않도록.</p>
 */
@Slf4j
@Component
    ...
    public void expireUnansweredReservations() {
        ...
        for (Long reservationId : expiredIds) {
            try {
                ...
                failed++;
                log.error("예약 자동 반려·환불 실패 - reservationId: {}, 원인: {}",
        ...
        if (failed > 0) {
            ...
            log.warn("예약 자동 반려 처리: 대상={}건, 완료={}건, 실패={}건 (실패분은 다음 실행에서 재시도)",
                    expiredIds.size(), processed, failed);
            return;
        }
