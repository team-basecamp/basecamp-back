 ...
 * 날씨 조회 비즈니스 로직.
 *
 * <p>DB 를 쓰지 않으므로 {@code @Transactional} 이 없다. 외부 HTTP 호출과 캐시 조회뿐이라
 * 트랜잭션을 열면 커넥션만 붙잡는다(CampService.registerCamp 가 지오코딩 때문에 트랜잭션을
 * 두지 않는 것과 같은 이유다).</p>
 */
@Service
...
public class WeatherService {
     ...
     *
     * <p>지역 하나가 실패해도 나머지는 그대로 내려간다. 실패한 지역은 값이 null 인 채로 포함되며,
     * 프론트가 "정보 없음"으로 표시한다. 16개 중 하나 때문에 위젯 전체가 사라지면 안 된다.</p>
     */
    public List<RegionWeatherResponseDto> getRegionWeathers() {
        return Arrays.stream(Region.values())
                .map(this::getRegionWeather)
                .toList();
    }
    ...
    private RegionWeatherResponseDto getRegionWeather(Region region) {
        ...
        CurrentWeatherResponse response = weatherClient.getCurrentWeather(region.getMapX(), region.getMapY());
     ...
     * 캠핑장의 예약 기간 날씨를 조회한다.
     *
     * <p>예보 범위(5일) 밖의 날짜는 결과에 담기지 않는다. "먼 미래라 예보가 없음"은 오류가 아니므로
     * 예외를 던지지 않고 그 날짜를 생략한다. 좌표가 없는 캠핑장(지오코딩 실패)도 빈 리스트다.</p>
     */
    public CampWeatherResponseDto getCampWeather(Long campId, LocalDate checkInDate, LocalDate checkOutDate) {
     ...
     *
     * <p>기온은 평균 낼 수 있어도 날씨 상태("맑음"/"비")와 아이콘은 평균이 불가능하다.
     * 그래서 하루의 <b>대표 시각(정오에 가장 가까운 항목)</b> 하나를 통째로 골라 쓴다.
     * 캠핑은 낮 시간대가 중심이므로 새벽보다 정오가 그날을 더 잘 대표한다.</p>
     */
    private List<CampWeatherResponseDto.WeatherDay> toDailyForecast(ForecastResponse response) {
        ...
        return response.getList().stream()
                ...
                .map(entry -> CampWeatherResponseDto.WeatherDay.of(entry.getKey(), pickRepresentative(entry.getValue())))
                .toList();
    }
    ...
    private ForecastResponse.Item pickRepresentative(List<ForecastResponse.Item> items) {
        return items.stream()
                .min(Comparator.comparingInt(item ->
                        Math.abs(toKstDateTime(item.getDt()).getHour() - REPRESENTATIVE_HOUR)))
                .orElse(items.get(0));
    }
