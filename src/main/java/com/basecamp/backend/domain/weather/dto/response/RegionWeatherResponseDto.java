 ...
 * 홈페이지 시/도별 날씨 위젯 응답.
 *
 * <p>OpenWeatherMap 응답({@link CurrentWeatherResponse})을 그대로 내려주지 않고 이 DTO 로 변환한다.
 * 외부 API 의 구조(main.temp, weather[0].icon)를 프론트가 알게 되면, 나중에 날씨 API 를 교체할 때
 * 프론트까지 함께 깨진다. 변환 계층을 두어 외부 계약 변경이 우리 API 계약으로 새지 않게 막는다.</p>
 *
 * <p>{@code @Jacksonized} 는 캐시(Redis) 에 저장된 JSON 을 다시 객체로 되살리기 위해 필요하다.
 * {@code @Builder} 만 있으면 Jackson 이 이 객체를 만들 수단이 없어 역직렬화에 실패한다.</p>
 */
@Getter
@Builder
@Jacksonized
public class RegionWeatherResponseDto {
     ...
     * Region + 외부 응답 → 응답 DTO.
     *
     * <p>외부 응답의 어느 필드든 없을 수 있다고 가정한다. 날씨는 부가 정보이므로,
     * 일부 값이 비어도 지역명만이라도 내려주는 편이 위젯 전체가 사라지는 것보다 낫다.</p>
     */
     ...
     *
     * <p>제공자가 새 코드를 추가해도 문구가 비어버리지 않도록 두 단계로 둔다.</p>
     */
     ...
     *
     * <p>비가 오면서 안개가 끼는 식으로 요소가 여러 개일 수 있어 배열로 오지만, 위젯에는 대표 하나만 쓴다.
     * 반대로 <b>배열이 비어 있거나 아예 없을 수도</b> 있으므로 get(0) 을 그냥 호출하지 않는다.</p>
     */
