 ...
 * 시/도별 날씨 조회 캐시.
 *
 * <p>날씨는 외부(OpenWeatherMap)가 가진 데이터를 잠시 빌려온 것이고 몇 분만 지나도 낡는다.
 * 우리 서비스의 진실 데이터가 아니므로 DB(MySQL)가 아니라 TTL 이 붙는 캐시에 둔다.
 * 오래된 항목은 TTL 로 자동 정리되므로 별도의 배치 삭제가 필요 없다.</p>
 *
 * <p><b>키 하나에 지역 하나의 날씨를 통째로(JSON) 담는다.</b> 필드별로 쪼개면 조회가 4배로 늘고,
 * 일부 필드만 만료돼 "기온은 없는데 아이콘은 맑음" 같은 반쪽 데이터가 생긴다.</p>
 *
 * <p><b>장애 정책:</b> 읽기·쓰기 모두 실패해도 예외를 던지지 않는다(fail-soft).
 * 캐시를 못 써도 외부 API 를 직접 부르면 되므로, Redis 장애가 날씨 기능을 멈출 이유가 없다.
 * ({@code TokenBlacklistCache} 가 쓰기 실패를 전파하는 것과 대비된다 — 폐기됐어야 할 토큰이
 * 살아나면 안 되기 때문이다. 같은 캐시라도 담는 데이터의 성격에 따라 정책이 다르다.)</p>
 */
@Slf4j
@Component
