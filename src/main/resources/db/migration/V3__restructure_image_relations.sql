...
--  DB      : MySQL 8.0+
--  변경     : 1) users.provider_id 제거 (유지)
--            2) 이미지 참조 방향 반전
--               기존) user_images.user_id -> users        (user_images 가 FK 보유)
--               변경) users.image_id      -> user_images  (users 가 FK 보유, 1:1)
...
-- -------------------------------------------------------------
-- 2. 이미지 구조 재설계 (참조 방향 반전)
--    user_images 는 이제 "공용 이미지 저장소"가 되고,
--    이를 참조하는 쪽(users / post_images / review_images)이 FK 를 갖는다.
-- -------------------------------------------------------------

-- 2-1) users 에 image_id(1:1 FK) 추가 후, 기존 프로필 이미지를 백필
--      UNIQUE 로 "회원당 이미지 1개" 를 유지.
--      (image_id 는 NULL 허용 = 프로필 없는 회원도 존재 가능. 이전과 동일한 의미)
ALTER TABLE users
    ADD COLUMN image_id BIGINT NULL
        COMMENT '프로필 이미지 (user_images 참조, 회원당 1개)' AFTER email,
...
UPDATE users u
JOIN user_images ui ON ui.user_id = u.user_id
SET u.image_id = ui.image_id;

-- 2-2) user_images 에서 users 로 향하던 관계 제거 (방향 반전 완료)
--      결과: user_images(image_id, image_url, created_at, updated_at)
...
-- -------------------------------------------------------------
-- 3. post_images : 게시글 <-> 이미지 중간 테이블 (1 게시글 : N 이미지)
--    posts 가 user_images 를 List 로 갖도록 하는 연결 테이블.
-- -------------------------------------------------------------
...
-- -------------------------------------------------------------
-- 4. review_images : 리뷰 <-> 이미지 중간 테이블 (1 리뷰 : N 이미지)
-- -------------------------------------------------------------
