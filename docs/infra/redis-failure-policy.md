...
> 한 줄 요약: **조회는 fail-open(통과), 쓰기는 예외 전파(롤백).** Redis 는 조회 캐시일 뿐이고, 진실의 원천(source of truth)은 항상 MySQL 이다.
...
| **쓰기** `blacklist` / `revoke` / `clear` | **예외 전파** | 예외를 그대로 던져 호출 트랜잭션을 롤백 | 조용히 넘기면 "로그아웃/제재했는데 토큰이 살아 있는" 상태가 된다 |
