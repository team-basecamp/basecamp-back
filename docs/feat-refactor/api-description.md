...
> 각 블록은 탭 구분(TSV)이므로 그대로 복사해 Excel에 붙여넣으면 8개 컬럼으로 분리된다.

## 컬럼 정의

| 컬럼 | 설명 |
|------|------|
| HTTP 메서드 경로 | 클래스 `@RequestMapping` + 메서드 매핑을 결합한 전체 경로 |
...
## perish95 — reservation / review / payment (15개)
...
## JONGBEEN_LEE — camp / wishlist / weather (16개)
...
POST /api/v1/camps/register	캠핑장주	camp	캠핑장 등록	O(ROLE_CAMP_OWNER)	body: {facltNm @NotBlank @Size(2~100), addr1 @NotBlank @Size(max=200), tel @NotBlank @Pattern(전화번호), induty @NotBlank @Size(max=100), price:Integer @NotNull @Min(0), addr2 @Size(max=200), gnrlSiteCo/autoSiteCo/glampSiteCo:Integer @Min(0) @Max(10000), lineIntro @Size(max=500), firstImageUrl @Size(max=255), homepage @URL(http/https) @Size(max=255)}	201: CampDetailResponseDto	INVALID_INPUT_VALUE(400), ACCESS_DENIED(403, ownerId null 또는 <=0), UNAUTHORIZED(401)
PATCH /api/v1/camps/{campId}	캠핑장주	camp	캠핑장 정보 수정	O(ROLE_CAMP_OWNER)	path: campId(Long), body: CampUpdateRequest(전 필드 선택, 제약은 등록과 동일)	200: CampDetailResponseDto	CAMP_NOT_FOUND(404), CAMP_NOT_ACCESSED(403, 소유자 불일치), INVALID_INPUT_VALUE(400), ACCESS_DENIED(403), UNAUTHORIZED(401)
DELETE /api/v1/camps/{campId}	캠핑장주	camp	캠핑장 삭제(소프트 삭제)	O(ROLE_CAMP_OWNER)	path: campId(Long)	204: 본문 없음	CAMP_NOT_FOUND(404), CAMP_NOT_ACCESSED(403, 소유자 불일치), ACCESS_DENIED(403), UNAUTHORIZED(401)
...
작성 중 발견한, 팀에서 한 번 논의해볼 만한 항목들.

1. **camp 도메인의 이중 응답 봉투** — camp만 `ApiResponse` 봉투 안에 다시 `CampDetailResponseDto`/`CampListResponseDto`(`resultCode`/`resultMsg`/`data`)라는 자체 봉투가 들어간다. 프론트는 이 도메인에서만 `data.data`로 접근해야 한다. 다른 도메인과 응답 형태가 다르므로 의도된 설계인지 확인 필요.
2. **`GET /api/v1/camps` 전체 조회에 페이징이 없다** — `campRepository.findAll()`로 전량 반환한다. 데이터가 커지면 문제가 될 수 있다.
3. **날씨 연동은 fail-soft** — `WeatherClient`가 외부 API 실패 시 예외 대신 null을 반환한다. 그래서 날씨 API에는 외부 연동 실패에 대한 ErrorCode가 없고, 대신 빈 리스트나 null 필드로 응답한다. 프론트가 이를 구분할 수단이 없다.
4. **`PAYMENT_AMOUNT_MISMATCH`(P009)는 정의만 되어 있고 throw되지 않는다** — `PaymentService.syncFromPortOne`이 결제를 FAILED로 기록하고 자동취소한 뒤 200 응답의 `failureReason`에 담는다(트랜잭션 롤백 방지 목적, 코드 주석에 명시). 의도된 동작이지만 ErrorCode 목록만 보면 오해하기 쉽다.
5. **`IMAGE_SIZE_EXCEEDED`(413)의 리뷰 경로 처리 미확인** — post 경로에서는 `GlobalExceptionHandler`가 `MaxUploadSizeExceededException`을 변환해 발생시키는 것을 확인했으나, 리뷰 이미지 업로드 경로에서 동일하게 동작하는지는 확인하지 못했다.
6. **`MyProfileResponse`에 role 필드가 없다** — Swagger 설명에는 권한이 언급되어 있으나 실제 DTO에는 `userId`/`email`/`nickname`/`profileImageUrl`/`provider`/`createdAt`만 있다. 문서와 구현 중 한쪽을 맞춰야 한다.
7. **권한명 표기** — 캠핑장주는 `ROLE_CAMP_OWNER`다. 팀 내에서 `ROLE_OWNER`로 지칭되는 경우가 있어 혼동 주의.
